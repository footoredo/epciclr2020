...
def add_extra_flags(parser):
    ...
    parser.add_argument('--initial-population', type=int, default=4)
...
def train_epc(arglist):
    ...
    for s in range(num_stages - 1):
        ...
        if not cooperative:
            for i in range(k):
                for j in range(k):
                    print("Training seed-{} (from wolf seed {} and sheep seed {}) ...".format(n, wolf_indices[i], sheep_indices[j]))
                    arglist.save_dir = join_dir(stage_dir, "seed-{}".format(n))
                    cur_dirs.append(arglist.save_dir)
                    arglist.wolf_init_load_dirs = [last_dirs[wolf_indices[i]]]
                    arglist.sheep_init_load_dirs = [last_dirs[sheep_indices[j]]]
                    mix_match(copy.deepcopy(arglist), double=False)
                    n += 1
            assert n == k * k
        else:
            raise ValueError
